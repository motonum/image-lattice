...
export default function Grid({ rows, cols, cells, updateCell, replaceCells }: Props) {
  const N = rows * cols

  const handleRemove = (index: number) => {
    const cell = cells[index]
    if (cell?.src && cell.src.startsWith && cell.src.startsWith('blob:')) {
      try {
    ...
    updateCell(index, { src: undefined, fileName: undefined, width: undefined, height: undefined, label: undefined })
  }
  ...
  const onFileDrop = async (e: React.DragEvent, index: number) => {
    ...
    if (files.length > 1) {
      ...
      const emptyIndices = cells.map((c, i) => ({ c, i })).filter(x => !x.c?.src).map(x => x.i)
  ...
  const onFileInput = async (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
  ...
  const handleFileLoad = async (file: File, index: number) => {
    ...
    if (lower.endsWith('.tif') || lower.endsWith('.tiff') || file.type === 'image/tiff') {
      ...
      try {
        ...
        updateCell(index, { src, fileName: name, width, height, label: stripExt(name) })
      } catch (err) {
      ...
      img.src = url
      img.onload = () => {
        updateCell(index, { src: url, fileName: name, width: img.naturalWidth, height: img.naturalHeight, label: stripExt(name) })
      }
    }
  }
  ...
  const renderCell = (i: number) => {
    ...
    return (
      ...
      >
        ...
        >
          {cell?.src ? (
            <div className="thumb" draggable onDragStart={(e) => onDragStart(e, i)}>
              ...
              />
              <button type="button" className="remove-btn" onClick={() => handleRemove(i)}>削除</button>
            </div>
          ) : (
            <div className="placeholder">
              <div>Drop image or</div>
              <input type="file" accept="image/*,.tif,.tiff" multiple onChange={(e) => onFileInput(e, i)} />
            </div>
          )}
        </div>
      </div>
