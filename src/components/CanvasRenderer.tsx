...
const CanvasRenderer = forwardRef<CanvasHandle | null, Props>(({ rows, cols, cells, gap, fontSize, preview = false, labelMode = 'overlay' }, ref) => {
  ...
  const totalHeight = rowHeights.reduce((a, b) => a + b, 0) + gap * Math.max(0, rows - 1) + labelReserve * (labelMode === 'overlay' ? 0 : rows)
          ...
          try {
            if (labelMode === 'above') {
              ...
              if (cell?.label) {
                ...
                ctx.drawImage(img, x, y + labelReserve, img.naturalWidth || w, img.naturalHeight || h)
            } else {
              // 'below' or 'overlay'
